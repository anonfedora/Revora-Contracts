...
mod test_reconciliation_completeness {
    use super::*;
    ...
    #[test]
    fn set_rounding_mode_emits_event() {
        let (env, client, issuer, token, _) = setup_with_offering();
        let before = env.events().all().len();
        client.set_rounding_mode(&issuer, &symbol_short!("def"), &token, &RoundingMode::Truncation);
        assert!(env.events().all().len() > before);
    }
    ...
    fn set_admin_emits_event() {
        ...
        // EVENT_ADMIN_SET is emitted by initialize via the event in set_admin code path
        assert!(env.events().all().len() > before, "initialize must emit EVENT_ADMIN_SET");
    }
    ...
    fn init_multisig_emits_event() {
        ...
        let admin = Address::generate(&env);
        client.initialize(&admin, &None, &None);
        let owners = soroban_sdk::vec![&env, Address::generate(&env), Address::generate(&env)];
        let before = env.events().all().len();
    ...
    #[test]
    fn register_meta_signer_key_emits_event() {
        let (env, client, _issuer, _token, _) = setup_with_offering();
        let holder = Address::generate(&env);
        let pub_key = soroban_sdk::BytesN::from_array(&env, &[0; 32]);
        let before = env.events().all().len();
        client.register_meta_signer_key(&holder, &pub_key);
        assert!(env.events().all().len() > before);
    }

    #[test]
    fn set_meta_delegate_emits_event() {
        let (env, client, issuer, token, _) = setup_with_offering();
        let holder = Address::generate(&env);
        let delegate = Address::generate(&env);
        let before = env.events().all().len();
        client.set_meta_delegate(&issuer, &symbol_short!("def"), &token, &delegate);
        assert!(env.events().all().len() > before);
    }
}
}
